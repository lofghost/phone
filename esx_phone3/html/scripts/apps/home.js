(function(){
	...
	app.updateCounters = function() {
		...
		Phone.twitter.tweets.forEach(tweet => {
			const lastClosed = Phone.twitter.lastClosed;
			if (!lastClosed) {
				return;
			}

			if (lastClosed < tweet.time) {
				tweets += 1;
			}
		});
		...
		if(tweets == 0) {
			...
			$('#app-home .menu-icon[data-app="twitter"] .counter').text(tweets).show();
		}

		if (Phone.settings.version !== Phone.currentVersion) {
			$('#app-home .menu-icon[data-app="settings"] .counter').text(1).show();
		} else {
			$('#app-home .menu-icon[data-app="settings"] .counter').hide();
		}
	}
