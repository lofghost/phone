...
(function(){
	...
	app.update = function(data){
        //('UPDATING, DATA IS ' + data);
		if(data){
            let type = data.type;
            //('UPDATING, TYPE IS ' + type);
			...
			//('Not updating. Only updating every '+ app.updateTime/1000 +' seconds.');
		}
	...
	function startLooking(){
		//('Looking for plates from app.vehicles');
		$.post('http://esx_phone3/search_nearby', JSON.stringify(app.vehicles));
	}

	function searchForPlateNearby(plate){
		//('Plate is: "' + plate + '"');
	...
	function updateCurrentVehicle(){
		if(!app.currentVehicle){
			if(app.currentActiveVehiclePlate){
				let plate = app.currentActiveVehiclePlate;
				//('Well. We do have a currently active vehicle plate: "'+ plate + '"');
				searchForPlateNearby(plate);
			}
			return false;
		}
	...
	function updateInfo(){
        ...
        // //('currentVehicle:' + JSON.stringify(app.currentVehicle));
        // //('lock: ' + JSON.stringify(app.currentVehicle.lock));
        // //('veh: ' + JSON.stringify(app.currentVehicle.vehicle));
        // //('typeof: ' + typeof app.currentVehicle.vehicle + typeof app.currentVehicle.lock);
        // //('Updating info on display');
        ...
        //('Current vehicle: ');
        //(JSON.stringify(app.currentVehicle));
	...
	function toggleVehicleEngine(){
		//('Toggling engine. Once');
		if(!app.currentVehicle){
			return false;
		}
		//('Toggling engine. Twice');
	...
	function updateCarList(){
		...
		updateState();
		//('Updating car list from database.');
		clearSelectingCar();
		setTimeout(function(){
